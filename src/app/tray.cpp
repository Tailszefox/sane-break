...
void StatusTrayWindow::buildPostponeMenu() {
  ...
  // The maximum is deliberately far above a work session here, which would produce a
  // dozen preset entries. Cap the ladder at one session and leave the rest to "Custom…".
  QList<int> presets = postponePresets(std::min(maxMinutes, smallEveryMinutes));
